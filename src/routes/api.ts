...
async function readSubmitBody(c: Context): Promise<SubmitBody> {
  ...
  if (contentType.includes('form')) {
    ...
    // `curl -d "https://…"` is form-encoded with the URL as the only key and no value.
    // The README shows exactly that, so honour it rather than demand a url= field.
    if (typeof body.url !== 'string') {
      const keys = Object.keys(body);
      if (keys.length === 1 && /^https?:\/\//i.test(keys[0]!) && body[keys[0]!] === '') body.url = keys[0]!;
    }
    return {
      url: typeof body.url === 'string' ? body.url : undefined,
      tags: parseTags(body.tags),
